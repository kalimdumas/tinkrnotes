...
export function AllStacksBoard(props: {
  onOpenNote: (noteId: number) => void;
  onOpenStack: (stackId: number) => void;
}) {
  const [stackSort, setStackSort] = createSignal<StackSortKey>('created');
  const [noteSort,  setNoteSort]  = createSignal<NoteSortKey>('created');
  const [categories, { refetch }] = createResource(listCategories);

  onMount(async () => {
    const mod = await import('../lib/data'); await mod.seedDefaultCategories(); await refetch();
  });
    ...
    <section class="flex-1 p-4 overflow-auto">
      <div class="mb-3 flex items-center gap-4">
        ...
        <div class="ml-auto flex items-center gap-3">
          ...
          <select class="border rounded px-2 py-1" value={stackSort()} onInput={e => setStackSort(e.currentTarget.value as StackSortKey)}>
            <option value="created">Created (old → new)</option>
            <option value="updated">Last edited</option>
            <option value="title">Title (A→Z)</option>
          </select>
          <label class="text-sm ml-4">Sort notes</label>
          <select class="border rounded px-2 py-1" value={noteSort()} onInput={e => setNoteSort(e.currentTarget.value as NoteSortKey)}>
            <option value="created">Created (old → new)</option>
            <option value="updated">Last edited</option>
            <option value="title">Title (A→Z)</option>
          </select>
      ...
      <div class="board" style="display:grid; grid-auto-flow:column; grid-auto-columns:20rem; gap:16px; align-items:start; overflow-x:auto; padding:8px 12px 12px;">
